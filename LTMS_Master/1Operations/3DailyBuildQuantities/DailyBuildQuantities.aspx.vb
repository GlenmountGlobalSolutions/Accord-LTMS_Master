...
#Region "Event Handlers"
    ...
    Private Sub GetEndLotUpdateStatus()
        Dim oSqlParameter As SqlParameter
        Dim colParameters As New List(Of SqlParameter)
        ...
        Try
            ...
            oSqlParameter = New SqlParameter("@SetexSchedule", SqlDbType.VarChar, 1)
            oSqlParameter.Value = rbOrders.SelectedValue
            colParameters.Add(oSqlParameter)

            oSqlParameter = New SqlParameter("@StatusBP1", SqlDbType.VarChar, 100)
            oSqlParameter.Direction = ParameterDirection.Output
            colParameters.Add(oSqlParameter)

            oSqlParameter = New SqlParameter("@StatusBP2", SqlDbType.VarChar, 100)
            oSqlParameter.Direction = ParameterDirection.Output
            colParameters.Add(oSqlParameter)

            oSqlParameter = New SqlParameter("@StatusBP3", SqlDbType.VarChar, 100)
            oSqlParameter.Direction = ParameterDirection.Output
            colParameters.Add(oSqlParameter)

            colOutput = DA.ExecSP("procGetLastEndLotProduced", colParameters)

            For Each oParameter In colOutput
                With oParameter
                    If .Direction = ParameterDirection.Output And .ParameterName = "@StatusBP1" Then
                        message1 = oParameter.Value.ToString()
                    ElseIf .Direction = ParameterDirection.Output And .ParameterName = "@StatusBP2" Then
                        message2 = oParameter.Value.ToString()
                    ElseIf .Direction = ParameterDirection.Output And .ParameterName = "@StatusBP3" Then
                        message3 = oParameter.Value.ToString()
                    End If
                End With
            Next

            If message1 <> "" Then
                Master.tMsg("", message1 + "<br>" + message2)
