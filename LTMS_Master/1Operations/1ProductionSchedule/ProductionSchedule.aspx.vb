...
#Region "private methods"
    ...
    Private Sub cmdMoveDlg_Click(sender As Object, e As System.EventArgs) Handles cmdMoveDlg.Click
        Try
            If hidNodeLevel.Value = "1" Then
                MoveLot()
            ElseIf hidNodeLevel.Value = "2" Then
                MoveSubLot()
            Else
                '
            End If
            ...
            If (Me.tbDay.Text.Length > 0) Then
                ...
                If (ds IsNot Nothing) AndAlso (ds.Tables.Count >= 1) Then
                    ...
                    If (ds.Tables.Count >= 1 And CurrentTableIndex < NumberOfLastTable) Then
                        bResult = bResult And PopulateTreeView(treeMon, ds.Tables(0))
                        lblDateMon.Text = ds.Tables(0).Rows(0).ItemArray(1).ToString
                        lblTotalMon.Visible = True
                        'CurrentTableIndex = CurrentTableIndex + 1
                        lblDateMon.Visible = True
                        lblTotalMon.Visible = True
                        CurrentTableIndex += 1
                    Else
                        treeMon.Nodes.Clear()
                        lblDateMon.Visible = False
                        lblTotalMon.Visible = False
                        'Label222.Visible = False
                    End If
                    If (ds.Tables.Count >= 1 And CurrentTableIndex < NumberOfLastTable) Then
                        bResult = bResult And PopulateTreeView(treeTue, ds.Tables(1))
                        lblDateTue.Text = ds.Tables(1).Rows(0).ItemArray(1).ToString
                        lblDateTue.Visible = True
                        lblTotalTue.Visible = True
                        CurrentTableIndex += 1
                    Else
                        treeTue.Nodes.Clear()
                        lblDateTue.Visible = False
                        lblTotalTue.Visible = False
                        'Label2.Visible = False
                    End If
                    If (ds.Tables.Count >= 2 And CurrentTableIndex < NumberOfLastTable) Then
                        bResult = bResult And PopulateTreeView(treeWed, ds.Tables(2))
                        lblDateWed.Text = ds.Tables(2).Rows(0).ItemArray(1).ToString
                        lblDateWed.Visible = True
                        lblTotalWed.Visible = True
                        CurrentTableIndex += 1
                    Else
                        treeWed.Nodes.Clear()
                        lblDateWed.Visible = False
                        lblTotalWed.Visible = False
                        ' Label3.Visible = False
                    End If
                    If (ds.Tables.Count >= 3 And CurrentTableIndex < NumberOfLastTable) Then
                        bResult = bResult And PopulateTreeView(treeThu, ds.Tables(3))
                        lblDateThu.Text = ds.Tables(3).Rows(0).ItemArray(1).ToString
                        lblDateThu.Visible = True
                        lblTotalThu.Visible = True
                        CurrentTableIndex += 1
                    Else
                        treeThu.Nodes.Clear()
                        lblDateThu.Visible = False
                        lblTotalThu.Visible = False
                        ' Label4.Visible = False
                    End If
                    If (ds.Tables.Count >= 4 And CurrentTableIndex < NumberOfLastTable) Then
                        bResult = bResult And PopulateTreeView(treeFri, ds.Tables(4))
                        lblDateFri.Text = ds.Tables(4).Rows(0).ItemArray(1).ToString
                        lblDateFri.Visible = True
                        lblTotalFri.Visible = True
                        CurrentTableIndex += 1
                    Else
                        treeFri.Nodes.Clear()
                        lblDateFri.Visible = False
                        lblTotalFri.Visible = False
                        'Label5.Visible = False
                    End If
                    If (ds.Tables.Count >= 5 And CurrentTableIndex < NumberOfLastTable) Then
                        bResult = bResult And PopulateTreeView(treeSat, ds.Tables(5))
                        lblDateSat.Text = ds.Tables(5).Rows(0).ItemArray(1).ToString
                        lblDateSat.Visible = True
                        lblTotalSat.Visible = True
                        CurrentTableIndex += 1
                    Else
                        treeSat.Nodes.Clear()
                        lblDateSat.Visible = False
                        lblTotalSat.Visible = False
                        ' Label6.Visible = False
                    End If
                    If (ds.Tables.Count >= 6 And CurrentTableIndex < NumberOfLastTable) Then
                        bResult = bResult And PopulateTreeView(treeSun, ds.Tables(6))
                        lblDateSun.Text = ds.Tables(6).Rows(0).ItemArray(1).ToString
                        lblDateSun.Visible = True
                        lblTotalSun.Visible = True
                        CurrentTableIndex += 1
                    Else
                        treeSun.Nodes.Clear()
                        lblDateSun.Visible = False
                        lblTotalSun.Visible = False
                ...
                If (dt.Rows.Count > 0) Then
                    ...
                    For i = 0 To dt.Rows.Count - 1
                        ...
                        If ((strLot <> dt.Rows(i)(psGetSchedule.LotNumber8).ToString()) Or (shiftChange)) Then
                            ...
                            If strIsSetexSchedule = "0" And rbOrders.SelectedValue = "0" Then
    ...
    Private Sub DeleteLotOrSubLot()
        Try
            ...
            Else
                lot = hidNodeLot.Value.ToString().Trim()

                If (selectedNodeLevel = 1) Then

                    DeleteLot(lot.Replace("-", "").Substring(0, 8), status, message)
                ...
                ElseIf (selectedNodeLevel = 2) Then
                    ...
                    DeleteSubLot(lot.Replace("-", "").Substring(0, 10), status, message)
    ...
    Private Sub DeleteSubLot(ByVal seqnum As String, ByRef status As String, ByRef message As String)
        Dim oSqlParameter As SqlParameter
        Dim colParameters As New List(Of SqlParameter)
        Dim colOutput As List(Of SqlParameter)

        seqnum = seqnum.Replace("-", "")

        oSqlParameter = New SqlParameter("@SequenceNumber", SqlDbType.VarChar, 20)
    ...
    Private Sub SaveEdits()
        Try
            ...
            If (status.ToUpper() = "TRUE") Then
                Master.tMsg("Edit", "Sub-Lot with Sequence Number of: " + seqNum + " has been updated.")
            Else
                Master.tMsg("Edit", "Error - Sub-Lot with Sequence Number of: " + seqNum + " has NOT been edited. <BR>Error message: " + message + "<BR>Status message: " + status)
            End If
    ...
    '            colOutput = DA.ExecSP("procPSMoveLotProdSched30", colParameters)
    '        Else
    ...
    '            strProcToBeRun = "ALL Orders,  procPSMoveLot::"
    ...
    Private Sub MoveLot()
        ...
        Try
            ...
            lotNum = hidNodeLot.Value.Trim().ToString().Replace("-", "")
            ...
            ElseIf multilot.Length() = 2 Then
                If rbOrders.SelectedValue = "1" Then 'SETEX
                    ...
                    Else
                        'Do nothing
                    End If
                ElseIf rbOrders.SelectedValue = "0" Then 'Honda
    ...
    Private Sub MoveSubLot()
        ...
        Try
            ...
            Dim lotNum As String = ""
            Dim strBroadcastPoint As String
            Dim AdjacentLotNum As String = ""


            lotNum = hidNodeLot.Value.Trim().ToString().Replace("-", "").Substring(0, 10)
            strBroadcastPoint = lotNum.Substring(0, 1)
            AdjacentLotNum = hidLotNum.Value.Trim().ToString().Replace("-", "").Substring(0, 10)
            ...
            oSqlParameter = New SqlParameter("@BeforeAfter", SqlDbType.VarChar, 6)
            oSqlParameter.Value = Me.rblBeforeAfter.SelectedValue 'strBeforeAfter
            colParameters.Add(oSqlParameter)

            oSqlParameter = New SqlParameter("@AdjacentLot", SqlDbType.VarChar, 12)
            oSqlParameter.Value = AdjacentLotNum 'Me.hidNodeLot.Value.Replace("-", "") 'destLotNum.Replace("-", "").Substring(0, 10)
            colParameters.Add(oSqlParameter)
            ...
            If (rbOrders.SelectedValue = "1") Then
                'call the procedure to move in the production schedule and optionally shipping schedule
                colOutput = DA.ExecSP("procPSMoveSubLotProdSched", colParameters) 'correct new name
            Else
                'move the lot in the Honda Schedule, in other words move indexes, prod, honda
                colOutput = DA.ExecSP("procPSMoveSubLot", colParameters)  'correct new name
            End If
            ...
            If (status.ToUpper() = "TRUE") Then
                Master.tMsg("Move", " Lot Number: " + lotNum + " has been moved " + Me.rblBeforeAfter.SelectedValue + " " + AdjacentLotNum)
            Else
                Master.tMsg("Move", "Error - Lot Number: " + lotNum + " has NOT been moved. <BR>Error message: " + message + "<BR>Status message: " + status)
            End If
    ...
    Private Sub MoveLots_Rebind()
        Try
            UnCheckAllNodes()
            'needed???     hidBroadcastPointID.Value = ddlMoveBP.SelectedValue
            '' treeMove.DataBind()
        Catch ex As Exception
            Master.eMsg(ex.ToString())
        End Try
