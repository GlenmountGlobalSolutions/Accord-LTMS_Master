Public Class MaterialDispositionHistory
        ...
        Try
            ...
            Else
                ...
                For x = 0 To (selectedIDs.Count - 1)
                    ...
                    Dim sql As String = "DELETE FROM tblRMDMaterialDispositionHistory WHERE (DispositionDate >= CONVERT(DATETIME, '" + IDstartDT + "', 102))"
                    sql += " AND (DispositionDate < CONVERT(DATETIME, '" + IDendDT + "', 102))"

                    'execute sql
                    DA.ExecSQL(sql)

                    'log event
                    deleteLog.Add("Material Disposition History with Datetime " & id & " was deleted.")
                Next
...
#Region "Methods"
    ...
    Public Sub Textbox_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            log.Add("Material Disposition History Record with Datetime " & id & " saved. " & colName & " old value: " & oldVal & ", new value: " & newVal & ".")
    ...
    Private Function BuildSqlString(ByVal column As String, ByVal val As String, ByVal ID As String, ByVal dataType As String) As String
        ...
        Select Case dataType.ToLower()
            ...
            Case "nvarchar"        'this will result in "... = N'value' " string (N + quotes around value)
                sql = "UPDATE tblRMDMaterialDispositionHistory SET " & column & " = N'" & val & "' WHERE (DispositionDate >= CONVERT(DATETIME, '" + IDstartDT + "', 102))"
                sql += " AND (DispositionDate < CONVERT(DATETIME, '" + IDendDT + "', 102))"

            Case "int"            'this will result in "... = value " string (no quotes around value)
                sql = "UPDATE tblRMDMaterialDispositionHistory SET " & column & " = " & val & " WHERE (DispositionDate >= CONVERT(DATETIME, '" + IDstartDT + "', 102))"
                sql += " AND (DispositionDate < CONVERT(DATETIME, '" + IDendDT + "', 102))"
    ...
    Private Sub LoadDG()
        Try
            ...
            sql += " SELECT DispositionDate, ShiftStartDate, Shift, ReworkArea, TeamMember, LotNumber, VehicleLine, SeatStyle, Color, Component, SeatSerialNumber, Category, PartNumber, "
            sql += " DispositionReason, ChargeTo, ReturnQuantity, ScrapQuantity, TotalQuantity, Location, PartDescription"
            sql += " FROM tblRMDMaterialDispositionHistory"
            sql += " ORDER BY DispositionDate DESC"
