...
#Region "Methods"
    ...
    Function ReturnFrameCode(ByVal PartR As String, ByVal PartL As String, ByVal PartRR As String, ByVal ColorCode As String) As String
        Try
            ...
            If (ext.Equals("txt")) Then
                ...
                Using sReader = New StreamReader(path)
                    ...
                    If strEDICheck = "ISA*" Then
                        ...
                        Do Until sReader.Peek = -1
                            ...
                            Do Until strLine.Contains("LIN*") Or sReader.Peek = -1
                                ...
                                If strLine.Contains("LIN*") Then
                                    If (strLine.Contains("BP*811")) Then
                                    ...
                                    ElseIf (strLine.Contains("BP*815")) Then
                                        'get data from the line
                                        pos4 = GetNthIndex(strLine, "*", 4) + 1
                                    ...
                                    ElseIf (strLine.Contains("BP*821")) Then
                                        'get data from the line
                                        pos4 = GetNthIndex(strLine, "*", 4) + 1
                                        For i = 1 To multiListR.Count
                        ...
                        While j <= multiListR.Count + multiListL.Count + multiListRR.Count - 1

                            If multiListR.Count = 0 Then 'No rights then don't process
                                Exit While
                            End If
                            If multiListL.Count = 0 Then 'No lefts then don't process
                                Exit While
                            End If

                            If j < multiListR.Count Then
                                'get right(s)
                                Do Until strLine.Contains("LIN*") And strLine.Contains(multiListR(0).Item(rt))
                                    strLine = Trim(sReader.ReadLine())
                                Loop
                                CurrentPart = multiListR(0).Item(rt + 1)
                                'get the right framecode
                                If multiListRR.Count = 0 Then
                                    strFrameCodeFromFileCheck = ReturnFrameCode(multiListR(0).Item(rt), multiListL(0).Item(rt), "", multiListR(0).Item(1)) 'No rears
                                Else
                                    strFrameCodeFromFileCheck = ReturnFrameCode(multiListR(0).Item(rt), multiListL(0).Item(rt), multiListRR(0).Item(rt), multiListR(0).Item(1))
                                End If
                                rt += 2
                                DoingRightParts = True
                                DoingLeftParts = False
                                DoingRearParts = False
                            ElseIf j < multiListR.Count + multiListL.Count Then
                                'get left(s)
                                Do Until strLine.Contains("LIN*") And strLine.Contains(multiListL(0).Item(lt))
                                    strLine = Trim(sReader.ReadLine())
                                Loop
                                CurrentPart = multiListL(0).Item(lt + 1)

                                'get the left framecode
                                If multiListRR.Count = 0 Then
                                    strFrameCodeFromFileCheck = ReturnFrameCode(multiListR(0).Item(lt), multiListL(0).Item(lt), "", multiListR(0).Item(1)) 'No rears
                                Else
                                    strFrameCodeFromFileCheck = ReturnFrameCode(multiListR(0).Item(lt), multiListL(0).Item(lt), multiListRR(0).Item(lt), multiListR(0).Item(1))
                                End If
                                lt += 2
                                DoingRightParts = False
                                DoingLeftParts = True
                                DoingRearParts = False
                            ElseIf j < multiListR.Count + multiListL.Count + multiListRR.Count Then
                                'get rear(s)
                                If multiListRR.Count <> 0 Then
                                    Do Until strLine.Contains("LIN*") And strLine.Contains(multiListRR(0).Item(rr)) Or sReader.Peek = -1
                                        strLine = Trim(sReader.ReadLine())
                                    Loop
                                    CurrentPart = multiListRR(0).Item(rr + 1)
                                    'get the rear framecode
                                    If multiListRR.Count = 0 Then
                                        strFrameCodeFromFileCheck = ReturnFrameCode(multiListR(0).Item(rr), multiListL(0).Item(rr), "", multiListR(0).Item(1)) 'No rears
                                    Else
                                        strFrameCodeFromFileCheck = ReturnFrameCode(multiListR(0).Item(rr), multiListL(0).Item(rr), multiListRR(0).Item(rr), multiListR(0).Item(1))
                                    End If
                                    rr += 2
                                    DoingRightParts = False
                                    DoingLeftParts = False
                            ...
                            Do While strLine.Contains("FST*") And sReader.Peek <> -1
                                ...
                                If strFrameCodeFromFileCheck <> "99" And strFrameCodeFromFileCheck.Length > 0 Then
                                    strLineMod = "  " & strLotNumber2 & strFrameCodeFromFileCheck & "0" & " " & CurrentPart & "    " & strFrameCodeFromFileCheck(strFrameCodeFromFileCheck.Length - 1) & " xxxx" & " " & qty.ToString.PadLeft(5, System.Convert.ToChar("0")) & "   ELP 0" & strLotNumber
                        ...
                        'Confirm Qty
                        If multiListRR.Count = 0 Then 'No rears to confirm qty
                        ...
                        Else
                            If (TotalQtyRight = TotalQtyLeft And TotalQtyRight = TotalQtyRear) And (TotalWeekRight = TotalWeekLeft) Then
                                ...
                                Next
                            Else
                                If (TotalQtyRight <> TotalQtyLeft And TotalQtyRight = TotalQtyRear) Then
                                    tbIssues.Text = "Total Quanties of Parts in File Not Equal. Total Righthand: " + CStr(TotalQtyRight) + ",  Total Lefthand: " + CStr(TotalQtyLeft)
                                    tbFileContents.Text = ""
                                ElseIf TotalWeekRight <> TotalWeekLeft Then
                                    tbIssues.Text = "Total Weeks of Parts in File Not Equal. Total Righthand: " + CStr(TotalWeekRight) + ",  Total Lefthand: " + CStr(TotalWeekLeft)
