...
Public Class KDPlanProcessing
    ...
    Public Shared strMsg As String = ""
...
#Region "Events"
    ...
    Private Sub Page_PreRender(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.PreRender
        Try
            ...
            If lbFileContent.SelectedIndex > 0 Then
                ScriptManager.RegisterStartupScript(UpdatePanel2, UpdatePanel2.GetType(), "focuss", "setFocus2(" & lbFileContent.SelectedIndex.ToString() & ");", True)
            End If
        Catch ex As Exception
            Master.eMsg(ex.ToString())
        End Try
    ...
    Private Function NewKDPlanNValueMonth(sInput As String) As String
        ...
        'iMonthSelected = CInt(lbFileContent.SelectedValue.Substring(7, 2))
        'iYearSelected = CInt(lbFileContent.SelectedValue.Substring(0, 4))
...
#Region "Methods"
    ...
    Private Function ReturnNvalues(ByVal path As String) As ArrayList
        ...
        Dim strProdLines As String = ""
        Dim strPreviousProdLines As String = ""
        Dim LinesAreCombinedInFile As Boolean
        ...
        Try
            ...
            ElseIf (ext.Equals("xls") Or ext.Equals("lsx")) Then
                ...
                Using stream As FileStream = File.Open(path, FileMode.Open, FileAccess.Read)
                    ...
                    Using (excelReader)
                        ...
                        For Each row As DataRow In dsResult.Tables(0).Rows
                            If ((Not row("Prod Dt").ToString.ToUpper().Contains("Daily Total:")) AndAlso (row("Batch No").ToString().Trim().Length <> 0) AndAlso (row("KD Lot No").ToString().Trim().Length <> 0)) Then
                                strProdLines = row("Batch No").ToString().Substring(0, 1)
                                If (strPreviousProdLines <> strProdLines) And strPreviousProdLines <> "" And strProdLines <> "" And LinesAreCombinedInFile = False Then
                                    LinesAreCombinedInFile = True
                                End If
                                strPreviousProdLines = strProdLines
                            End If
                        Next
                ...
                If LinesAreCombinedInFile Then
                    strReturnNvalues.Add("N-1 " & Format(CDate(strModKDPlan), "MMMM dd yyyy") & " - Lines 1 and 2" & marker) '20211119 removed due to multiple line processing
                Else
                    strReturnNvalues.Add("N-1 " & Format(CDate(strModKDPlan), "MMMM dd yyyy") & " - Line " & strProdLine & marker) '20211119 removed due to multiple line processing
                End If
    ...
    Private Sub WriteKDPlanSelected(ByVal path As String, ByVal strNvalue As String)
        ...
        Dim sLotPick As String = ""
        Dim sLotPicker(30) As String
        Dim PriorityONDt As String = ""
        Dim PriorityONTime As String = ""
        Dim iUIDCounter As Integer
                            ...
                            If ((Not row("Prod Dt").ToString.ToUpper().Contains("DAILY TOTAL:")) AndAlso (row("KD Lot No").ToString().Trim().Length <> 0)) Then
                                ...
                                If strItem.Length = 4 Then
                                    ...
                                    If (ord + 2 < row.Table.Columns.Count) Then
                                        ...
                                        strItem = row(ord + 2).ToString().PadRight(7, Chr(46)) + "          "
                                ...
                                'If row("KD Lot No").ToString() = "MAP 01202202021701" Then
                                '    msg = "32103"
                                'End If
                                ...
                                PriorityONDt = row(" Priority ON  Dt").ToString()
                                PriorityONTime = row(" Priority ON Time").ToString()
                                ...
                                '20211019
                                strLine += " " & PriorityONDt & " " & PriorityONTime

                                'sLotPick += Right(strLine, 64)

                                'If Right(sLotPick, 64) = Right(strPreviousLine, 64) Then
                                '    sLotPick = sLotPick + "*"
                                'End If

                                sLotPick += Right(strLine, 82)

                                'If Right(sLotPick, 82) = Right(strPreviousLine, 82) Then
                                '    sLotPick = sLotPick + "."
                                'End If

                                sLotPick = sLotPick + "_UID" & iUIDCounter.ToString()
                                iUIDCounter += 1

                                strPreviousLine = sLotPick
                                sLotPick += vbCrLf + "^"
                                strLine += vbCrLf
                                ...
                                If (batchQty = QTY_PER_BATCH) Or (batchQty = 1 And lstModelExceptionIdentifierArray.Contains(row("MTOC").ToString().Trim())) Then
                                        ...
                                        sLotPicker = sLotPick.Split(Chr(94))

                                        For i = 0 To sLotPicker.Length - 1
                                            Dim liTest As New ListItem
                                        liTest.Value = sLotPicker(i)

                                        If sLotPicker(i) <> "" Then
                                                'If lbFileContent.Items.Contains(liTest) = False Then
                                                lbFileContent.Items.Add(liTest)
                                                'End If
                                            End If
                                        Next

                                        'StartLot.Text = lbFileContent.Items.Count.ToString
                                        sLotPicker.Clear(sLotPicker, 0, sLotPicker.Length)
        ...
        Finally
            If strKDEntireFile.Length <> 0 Then
                If Right(lblFileName.Text, 3) = "txt" Then
                    lbFileContent.Text = strKDEntireFile
                End If
            End If
        End Try
    End Sub
    ...
    Private Sub EnableControls()
        Try
            ...
            Else
                ...
                lbFileContent.Enabled = False
                cmdProcess.Enabled = False
            End If

            If lbFileContent.SelectedIndex < 0 And UCase(Right(lblFileName.Text, 3)) = "TXT" Then
                ...
                Else
                    cmdProcess.Enabled = False
                End If
            ElseIf lbFileContent.SelectedIndex > -1 Then
                cmdProcess.Enabled = True
            Else
                cmdProcess.Enabled = False
            End If
...
#Region "DTS File Validation "
    ...
    Private Function WriteFile(ByVal path As String) As Boolean
        ...
        Try
            ...
            'Dim sWriter As New StreamWriter(path, False)

            'open a new file for writing
            Using sWriter As New StreamWriter(path, False)
                'write file out
                For Each I As Integer In lbFileContent.GetSelectedIndices
                    sWriter.Write(lbFileContent.Items(I).Value.Substring(0, 82).Replace(Chr(46), " ") & vbCrLf)
                    'sWriter.Write(lbFileContent.Items(I).Value.Replace(Chr(46), " "))
                Next
            End Using
    ...
    Private Function ValidSequence(ByVal sequenceIsValid As Boolean) As Boolean
        ...
        Try
            ...
            Else
                strNfileDate = strNfileDate.Substring(0, strNfileDate.LastIndexOf("-") - 1)  '20211119 removed for multi line processing
                strNfileDate = CStr(CDate(strNfileDate))
...
#Region "Dialog modalProcess_.aspx"
    ...
    Private Sub PopulateLabels()
        'StartLotValue.Text = Right(lbFileContent.SelectedItem.ToString, 14)
        StartLotValue.Text = Mid(lbFileContent.SelectedItem.ToString, 51, 14)

        Dim str As String = ""
        Dim ii As Integer = 0

        For Each I As Integer In lbFileContent.GetSelectedIndices
            Dim CurrentItem As ListItem = lbFileContent.Items(I)
        Next

        ii = lbFileContent.GetSelectedIndices.Last
        EndLotValue.Text = Mid(lbFileContent.Items(ii).ToString, 51, 14)
    ...
    Private Sub cmdSearch_Click(sender As Object, e As EventArgs) Handles cmdSearch.Click
        ...
        Try
            ...
            For idx = 0 To lbFileContent.Items.Count - 1
                ' Match found: set as selected item and exit procedure
                If lbFileContent.Items(idx).Text.Trim.Substring(48, 14) = tbSearch.Text.Trim Then
                    'If Left(Right(lbFileContent.Items(idx).Text.Trim, 30), 12) = tbSearch.Text.Trim Then
                    lbFileContent.SelectedIndex = idx
            ...
            Next
            If i = 0 Then
                Master.eMsg("No match found for : " & tbSearch.Text.Trim)
            End If
        Catch ex As Exception
            Master.eMsg(ex.ToString())
        End Try
    ...
    Private Sub lbFileContent_PreRender(sender As Object, e As EventArgs) Handles lbFileContent.PreRender

    End Sub

    Private Sub lbFileContent_SelectedIndexChanged(sender As Object, e As EventArgs) Handles lbFileContent.SelectedIndexChanged

        PopulateLabels()

        If lbFileContent.SelectedIndex > -1 Then
            cmdProcess.Enabled = True
        Else
            cmdProcess.Enabled = False
        End If
